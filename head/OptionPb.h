...
{
public:
        NonCopyable() = default;  // 禁用默认构造
        ~NonCopyable() = default; // 禁用析构
...
struct AVFrameWrap : NonCopyable
{
        ...
        AVFrameWrap(AVFrameWrap &&temp);
        AVFrameWrap(AVFrameWrap &temp);
        ~AVFrameWrap();
        inline void operator=(AVFrameWrap &temp);
        inline void operator=(AVFrameWrap &&temp);
};
...
public:
        OptionPb();
        void pushAVframe(AVFrameWrap &frame);
        bool handleOpt(MY_OPTION_PB opt = PB_OPT_TEST);
        void stopTask();
...
private:
        ...
        std::queue<AVFrame *> _myAVFrames;

        tbb::concurrent_queue<AVFrameWrap> _queueFrame;
        // tbb::concurrent_queue<AVFrame *> _queueFrame;
